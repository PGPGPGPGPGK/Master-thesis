...
class HandCodedLaneFollower(object):
    ...
    def steer(self, frame, lane_lines):
        ...
        # curr_steering_angle, new_steering_angle, num_of_lane_lines,
        stb_stl = stabilize_steering_angle(
            self.curr_steering_angle, new_steering_angle, len(lane_lines))
        self._set_curr_steering_angle(stb_stl)
        #print("stabilitized angle: ", stb_stl)
    ...
    def _set_curr_steering_angle(self, new_steering_angle):
...
def stabilize_steering_angle(curr_steering_angle, new_steering_angle, num_of_lane_lines,
                             max_angle_deviation_two_lines=15, max_angle_deviation_one_lane=40):
    """
...
if __name__ == '__main__':
    ...
    logging.warning('Logging level set to %s' % args.loglevel.upper())
    ...
    try:
        ...
        for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
            ...
            stb_stl = lane_follower.follow_lane(frame)
            logging.info('stb_stl: ' + str(stb_stl))
            if type(stb_stl) is int and stb_stl is not None:
                pwm.setServoPwm('4', stb_stl)
                #x = 55-(abs(stb_stl-90)*0.66)
                PWM.setMotorModel(35, 35)
